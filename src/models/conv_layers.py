...
from torch_geometric.typing import NodeType, OptPairTensor, Adj, OptTensor, Size, PairTensor
...
from torch_geometric.nn import RGCNConv as BaseRGCNConv
...
@torch.jit._overload
def masked_edge_atten(edge_index, edge_mask):
    # type: (Tensor, Tensor) -> Tensor
    pass
...
def masked_edge_atten(edge_atten, edge_mask):
    if edge_atten is None:
        return None
    if isinstance(edge_atten, Tensor):
        return edge_atten[edge_mask, :]
    else:
        return masked_select_nnz(edge_atten, edge_mask, layout='coo')

class RGCNConv(BaseRGCNConv):
    def forward(self, x: Union[OptTensor, Tuple[OptTensor, Tensor]],
                edge_index: Adj, edge_type: OptTensor = None,
                edge_attr: OptTensor = None, edge_atten: OptTensor = None, size: Size = None):
        r"""
        Args:
        ...
        if self.num_blocks is not None:  # Block-diagonal-decomposition =====
            ...
            for i in range(self.num_relations):
                ...
                h = self.propagate(tmp, x=x_l, size=size, edge_atten=masked_edge_atten(edge_atten, edge_type == i))
            ...
            for i in range(self.num_relations):
                ...
                if x_l.dtype == torch.long:
                    out += self.propagate(tmp, x=weight[i, x_l], size=size, edge_atten=masked_edge_atten(edge_atten, edge_type == i))
                else:
                    h = self.propagate(tmp, x=x_l, size=size, edge_atten=masked_edge_atten(edge_atten, edge_type == i))
                    out = out + (h @ weight[i])
    ...
    def message(self, x_j: Tensor, edge_attr: OptTensor = None, edge_atten: OptTensor = None) -> Tensor:
