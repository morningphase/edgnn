...
from datasets import SynGraphDataset, Mutag, SPMotif, MNIST75sp, graph_sst2, LogFusionDataset


def get_data_loaders(data_dir, dataset_name, batch_size, splits, random_state, mode, parameters, mutag_x=False):
    multi_label = False
    """ assert dataset_name in ['Apache-1', 'Apache-2', 'Pgsql', 'ImageMagick-1', 'ImageMagick-2', 'Nginx', 'Proftpd', 'Redis', 'Vim'] """
    ...
    if dataset_name in ['Apache-1', 'Apache-2', 'Pgsql', 'ImageMagick-1', 'ImageMagick-2', 'Nginx', 'Proftpd', 'Redis', 'Vim']:
        dataset = LogFusionDataset(data_dir, dataset_name, False, mode, parameters)
        if dataset.mode == 'evasion':
        ...
        split_idx = get_random_split_idx(dataset, splits)
        loaders, test_set = get_loaders_and_test_set(batch_size, dataset=dataset, split_idx=split_idx)
        train_set = dataset[split_idx["train"].astype(np.int64)]

    # split_idx = dataset.get_idx_split() TODO:对于逃逸攻击，需要实现LogFusionDataset里面的get_idx_split()方法
    loaders, test_set = get_loaders_and_test_set(batch_size, dataset=dataset, split_idx=split_idx)
    train_set = dataset[split_idx["train"].astype(np.int64)]
    ...
    """ elif dataset_name == 'mutag':
        ...
        name='-'.join(dataset_name.split('_'))
    ...
    if not hasattr(test_set[0], "num_relations"):
        aux_info = {'deg': deg, 'multi_label': multi_label}
    else:
        aux_info = {'deg': deg, 'multi_label': multi_label, 'num_relations': test_set[0].num_relations.item()}
...
def get_loaders_and_test_set(batch_size, dataset=None, split_idx=None, dataset_splits=None):
    if split_idx is not None:
        ...
        train_loader = DataLoader(dataset[split_idx["train"].astype(np.int64)], batch_size=batch_size, shuffle=True)
        valid_loader = DataLoader(dataset[split_idx["valid"].astype(np.int64)], batch_size=batch_size, shuffle=False)
        test_loader = DataLoader(dataset[split_idx["test"].astype(np.int64)], batch_size=batch_size, shuffle=False)
        test_set = dataset.copy(split_idx["test"].astype(np.int64))  # For visualization
    else:
