...
from utils import Writer, CriterionClf, MLP, visualize_a_graph, save_checkpoint, load_checkpoint, get_preds, get_lr, set_seed, process_data
from utils import get_local_config_name, get_model, get_data_loaders, write_stat_from_metric_dicts, reorder_like, init_metric_dict, generate_all_cases
...
class GSAT(nn.Module):
    ...
    def __init__(self, clf, extractor, optimizer, scheduler, writer, device, model_dir, dataset_name, num_class, multi_label, random_state,
        ...
        self.criterion = CriterionClf(num_class, multi_label)
    ...
    def forward_pass(self, data, epoch, training):
        if data.edge_type is None:
            emb = self.clf.get_attention_emb(data.x, data.edge_index, batch=data.batch, edge_attr=data.edge_attr)
        else:
            emb = self.clf.get_attention_emb(data.x, data.edge_index, batch=data.batch, edge_attr=data.edge_attr, edge_type=data.edge_type)
        ...
        if not hasattr(data, 'edge_type'):
            clf_emb = self.clf(data.x, data.edge_index, data.batch, edge_attr=data.edge_attr, edge_atten=edge_att)
        else:
            clf_emb = self.clf(data.x, data.edge_index, data.batch, edge_attr=data.edge_attr, edge_atten=edge_att, edge_type=data.edge_type)
        clf_logits = self.clf.get_pred_from_emb(clf_emb, data.batch, data.edge_index)
...
def train_gsat_one_seed(local_config, data_dir, log_dir, mode, params, model_name, dataset_name, method_name, device, random_state):
    print('====================================')
    print('====================================')
...
def main():
    ...
    local_config['GSAT_config']['lr'] = float(args.lr)
    local_config['GSAT_config']['batch_size'] = int(args.batch_size)
    ...
    for case in all_cases:
        ...
        for random_state in range(num_seeds):
            log_dir = data_dir / dataset_name / dir /'logs' / (time + '-' + dataset_name + '-' + model_name + '-seed' + str(random_state) + '-' + args.lr + "-" + args.batch_size + "-" + args.hidden_size + "-" + args.dropout_p + "-" + args.extractor_dropout_p + '-' + method_name)
            hparam_dict, metric_dict = train_gsat_one_seed(local_config, data_dir, log_dir, mode, params, model_name, dataset_name, method_name, device, random_state)
            metric_dicts.append(metric_dict)

        log_dir = data_dir / dataset_name / dir /'logs' / (time + '-' + dataset_name + '-' + model_name + '-seed99-'+ args.lr + "-" + args.batch_size + "-" + args.hidden_size + "-" + args.dropout_p + "-" + args.extractor_dropout_p + '-' + method_name + '-stat')
        log_dir.mkdir(parents=True, exist_ok=True)
        writer = Writer(log_dir=log_dir)
